...
/*
  imgfs. Image filesystem. Based on bbfs
  2013 David Martinez Oliveira <dmo@papermint-designs.com>
...
int bb_truncate(const char *path, off_t newsize)
{
      ...
      if (ext)
	{
	  *ext = 0;
	  ext++;
	  snprintf (fname, PATH_MAX,"%s_%d.%s", tmp, (int)time (NULL), ext);
	}
      else
	snprintf (fname, PATH_MAX,"%s.%s", tmp, ext);
      
      log_msg ("\nRenaming to file %s\n", fname);
      sprintf (cmd,"cp '%s' '%s'", fpath, fname);
      log_msg ("\n CMD: %s\n", cmd);
      system (cmd);
      free (tmp);
    }
